...
def build_formation_memberships(stock_size, be_map):
    # For June year t: use June t size and Dec t-1 be/me
    member_2x3 = defaultdict(dict)     # FF 2x3 legs for factor construction
    member_char_3x3 = defaultdict(dict)  # DT characteristic buckets (size tercile x bm tercile)
    june_chars = []
    ...
    for y in range(1992, 2026):
        ...
        # candidate stocks: June size + Dec size + prior-year BE
        ...
        # FF factors: 2x3 independent sorts
        size_2 = assign_bins_rank(size_items, cuts=[0.5])
        bm_3_for_ff = assign_bins_rank(bm_items, cuts=[0.3, 0.7])

        # DT tests: 3x3 characteristics for cleaner within-group comparisons
