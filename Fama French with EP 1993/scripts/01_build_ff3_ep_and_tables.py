...
def load_earnings_calendar_year():
    earn_map = defaultdict(dict)
    date_map = defaultdict(dict)

    with open(cfg.NI_FILE, newline='', encoding='utf-8-sig') as f:
        r = csv.DictReader(f)
        for row in r:
            if row.get('Typrep') != 'A':
                continue
...
def build_formation_membership(stock_size, earn_map):
    member_2x3 = defaultdict(dict)
    member_5x5 = defaultdict(dict)
    june_chars = []
    ...
    for y in range(1992, 2026):
        ...
        for (m, stk), june_size in stock_size.items():
            ...
            ep = earn / dec_size
            # FF-style value sort uses strictly positive ratios
            if ep <= 0:
                continue
            ...
            ep_items.append((stk, ep))

        if not rec:
            continue

        size_2 = assign_bins_rank(size_items, cuts=[0.5])
        ep_3 = assign_bins_rank(ep_items, cuts=[0.3, 0.7])

        size_5 = assign_bins_rank(size_items, cuts=[0.2, 0.4, 0.6, 0.8])
        ep_5 = assign_bins_rank(ep_items, cuts=[0.2, 0.4, 0.6, 0.8])
        ...
        for stk, row in rec.items():
            ...
            if s2 is not None and e3 is not None:
                size_lbl = 'S' if s2 == 1 else 'B'
                ep_lbl = {1: 'L', 2: 'M', 3: 'H'}[e3]
                member_2x3[y][stk] = size_lbl + ep_lbl

            if s5 is not None and e5 is not None:
                member_5x5[y][stk] = (s5, e5)
...
def build_monthly_series(member_2x3, member_5x5, stock_ret, stock_size, rf, mkt):
    ...
    for m in months:
        ...
        for stk, (sq, eq) in members_5x5.items():
            by_25[(sq, eq)].append(stk)
    ...
    for i in range(1, 6):
        for j in range(1, 6):
            ...
            for m in months:
                ...
                if r is None or rf_m is None:
                    continue
                if fr['MKT_RF'] is None or fr['SMB'] is None or fr['HML_EP'] is None:
                    continue
                y.append(r - rf_m)
                x1.append(fr['MKT_RF'])
                x2.append(fr['SMB'])
...
def make_table_7_pricing_errors(reg_rows, port25_returns, rf, factor_rows):
    fac_means = {
        ...
        'HML_EP': mean([r['HML_EP'] for r in factor_rows if r['HML_EP'] is not None]),
    }

    reg_map = {(r['size_quintile'], r['ep_quintile']): r for r in reg_rows}
    months = cfg.all_months(cfg.RETURN_START, cfg.RETURN_END)
    ...
    for i in range(1, 6):
        for j in range(1, 6):
            ...
            fitted = (
                ...
                + reg['beta_hml_ep'] * fac_means['HML_EP']
            )
...
def main():
    ...
    earn_map = load_earnings_calendar_year()

    print('Building formation memberships...')
    member_2x3, member_5x5, june_chars = build_formation_membership(stock_size, earn_map)
    ...
    write_csv(
        cfg.OUTPUT_DIR / 'ff3_ep_june_characteristics.csv',
        ['form_year', 'Stkcd', 'size_june', 'size_dec_prev', 'earnings', 'ep', 'size2', 'ep3', 'leg_2x3', 'size5', 'ep5'],
        june_chars,
    )
    ...
    for m in months:
        ...
        leg_rows.append(row)
    write_csv(
        cfg.OUTPUT_DIR / 'ff3_ep_leg_returns.csv',
        ['Trdmnt', 'SL', 'SM', 'SH', 'BL', 'BM', 'BH', 'N_SL', 'N_SM', 'N_SH', 'N_BL', 'N_BM', 'N_BH'],
        leg_rows,
    )

    write_csv(
        cfg.OUTPUT_DIR / 'ff3_ep_factors_monthly.csv',
        ['Trdmnt', 'RF', 'RM', 'MKT_RF', 'SMB', 'HML_EP', 'SL', 'SM', 'SH', 'BL', 'BM', 'BH', 'N_SL', 'N_SM', 'N_SH', 'N_BL', 'N_BM', 'N_BH'],
        factor_rows,
    )
    ...
    write_csv(
        cfg.OUTPUT_DIR / 'ff3_ep_port25_monthly.csv',
        ['Trdmnt', 'size_quintile', 'ep_quintile', 'portfolio', 'vw_return', 'excess_return', 'n_stocks'],
        port25_rows,
    )
    ...
    write_csv(
        ...
        ['size_quintile', 'ep_quintile', 'avg_excess_return_pct', 'n_months'],
        t4,
    )
    write_csv(
        cfg.OUTPUT_DIR / 'table_5_25port_ff3_ep_regressions.csv',
        ['portfolio', 'size_quintile', 'ep_quintile', 'alpha_pct', 'beta_mkt', 'beta_smb', 'beta_hml_ep', 'nw12_t_alpha', 'nw12_t_mkt', 'nw12_t_smb', 'nw12_t_hml_ep', 'r2', 'n_months'],
        t5,
    )
    write_csv(
    ...
    write_csv(
        ...
        ['portfolio', 'size_quintile', 'ep_quintile', 'actual_avg_excess_pct', 'fitted_avg_excess_pct', 'pricing_error_pct', 'rmse_pct', 'mae_pct'],
        t7,
    )
    write_csv(
