...
def build_table_iii_md():
    ...
    order = [
        ...
        'M7_all3', 'M8_eplusp_nege', 'M9_all3_ep'
    ]
    coef_order = ['intercept', 'beta', 'ln_size', 'ln_be_me', 'eplus_p', 'neg_e']
    ...
    txt = []
    txt.append('## Table III. Fama-MacBeth Cross-sectional Regressions with E/P (NW12 t-stats)')
...
def build_table_iv_md():
    ...
    headers = ['BE/ME Decile', 'Avg Monthly Return (%)', 'Avg Size']
    out = []
    for r in rows:
        d = r['portfolio'].replace('M', '')
        out.append([d, f3(r['avg_monthly_return_pct']), f3(r['avg_size'])])
