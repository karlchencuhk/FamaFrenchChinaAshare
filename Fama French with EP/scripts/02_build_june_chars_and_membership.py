...
def load_net_profit_a_statement():
    earn = defaultdict(dict)
...
def main():
    ...
    earn_map = load_net_profit_a_statement()
    ...
    for y in range(1992, 2026):
        ...
        for stk, smap in stock_size.items():
            ...
            dec_prev = smap.get(f'{y - 1:04d}-12')
            ...
            eplus_p = None
            neg_e = None
            if dec_prev is not None and dec_prev > 0 and earn is not None:
                ep = earn / dec_prev
                if earn > 0:
                    eplus_p = ep
                    neg_e = 0.0
    ...
    with open(june_file, 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow([
            'form_year', 'form_month', 'Stkcd',
            'beta', 'size', 'ln_size',
            'book_equity', 'be_me', 'ln_be_me',
            'earnings', 'dec_me_prev', 'ep', 'eplus_p', 'neg_e',
        ...
        for k in sorted(june_chars.keys()):
            ...
            w.writerow([
                ...
                '' if row['earnings'] is None else f"{row['earnings']:.6f}",
                '' if row['dec_me_prev'] is None else f"{row['dec_me_prev']:.6f}",
                '' if row['ep'] is None else f"{row['ep']:.10f}",
                '' if row['eplus_p'] is None else f"{row['eplus_p']:.10f}",
                '' if row['neg_e'] is None else f"{row['neg_e']:.1f}",
