...
TARGET_DIRS = [
    ROOT / 'Fama French 1992' / 'output',
    ROOT / 'Fama French 1993' / 'output',
]

TEMPLATE = '''"""Auto-generated wrapper for __CSV_NAME__."""
from pathlib import Path
import csv
import sys
import html
...
def main():
    ...
    for out_dir in TARGET_DIRS:
        ...
        for csv_file in sorted(out_dir.glob('*.csv')):
            py_file = csv_file.with_suffix('.py')
            py_file.write_text(TEMPLATE.replace('__CSV_NAME__', csv_file.name), encoding='utf-8')
            created.append(py_file)
            print(f'Created: {py_file}')

    print(f'\nDone. Created {len(created)} Python files.')
